...
def process_message(
    ...
    logger.info(
        "[%s] START process_message | session=%s | lang=%s | msg='%s'",
        request_id, session_id, language, message[:80],
    )
    ...
    try:
        ...
        raise ChatbotException(
            ...
            {"input_language": language, "message_sample": message[:80]},
            request_id=request_id,
        ) from exc
    ...
    try:
        ...
        if not en_message.strip():
            en_message = message  # safety fallback
        logger.info("[%s] ✓ Translated: '%s'", request_id, en_message[:120])
    except Exception as exc:
        raise ChatbotException(
            ErrorCode.TRANSLATION_FAILED,
            f"Translation to English failed: {exc}",
            {"src": detected_language, "dest": "en", "message_sample": message[:80]},
            request_id=request_id,
        ) from exc
    ...
    if ctx.get("last_crop") and ctx["last_crop"].lower() not in en_message.lower():
        ...
        en_message_enriched = en_message
    logger.debug("[%s] Context-enriched message: '%s'", request_id, en_message_enriched[:120])
    ...
    try:
        ...
        season: Optional[str] = intent.get("season")
        logger.info(
            "[%s] ✓ Intent: %s | crops=%s | state=%s | season=%s",
            request_id, intent_type, crops, state, season,
        )
    except Exception as exc:
        raise ChatbotException(
            ErrorCode.INTENT_EXTRACTION_FAILED,
            f"Intent extraction failed: {exc}",
            {"english_text": en_message_enriched[:120]},
            request_id=request_id,
        ) from exc
    ...
    try:
        ...
        raise ChatbotException(
            ...
            {"intent_type": intent_type, "crops": crops, "state": state},
            request_id=request_id,
        ) from exc
    ...
    if detected_language != "en" and reply_en:
        try:
            logger.info(
                "[%s] STEP 5: Translating response back to %s",
                request_id, detected_language,
            )
            reply_final = translator.translate_text(reply_en, src="en", dest=detected_language)
            logger.info("[%s] ✓ Response translated", request_id)
        except Exception as exc:
            logger.warning(
                "[%s] Back-translation failed (%s); using English response: %s",
                request_id, detected_language, exc,
            )
            reply_final = reply_en
    else:
    ...
    audio_url: Optional[str] = None
    try:
        logger.info("[%s] STEP 6: Generating TTS audio", request_id)
        audio_path = text_to_speech.synthesize_speech(reply_final, detected_language)
        if audio_path:
            filename = os.path.basename(audio_path)
            audio_url = f"/api/chat/audio/{filename}"
            ...
            logger.warning("[%s] TTS returned no path", request_id)
    except Exception as exc:
        logger.warning("[%s] TTS generation failed: %s", request_id, exc)

    # ------------------------------------------------------------------
    # Step 8 – Build suggestions & persist to memory
    # ------------------------------------------------------------------
    suggestions = _make_suggestions(intent_type, crops, state)
